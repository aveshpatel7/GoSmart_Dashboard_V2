...
function showPage(name){
  ...
  if(titles[name]){$("pageTitle").textContent=titles[name][0];$("pageSub").textContent=titles[name][1]}
...
function renderOverview(){
  $("statTotal").textContent=DATA.all_nodes.length;$("statOnline").textContent=DATA.nodes.length;$("statOffline").textContent=Math.max(0,DATA.all_nodes.length-DATA.nodes.length);$("statAlerts").textContent=buildAlerts().length;
...
function getOptimistic(node){if(!optimisticState[node])optimisticState[node]={relays:{},relayAt:{},speed:null,speedAt:0};return optimisticState[node]}
function channelRecord(node,i){const c=rec(node).channels||{};return c[String(i)]||c[i]||{}}
function actualRelay(node,i){const c=channelRecord(node,i);return String(c.state??c.status??"OFF").toUpperCase()==="ON"?"ON":"OFF"}
function actualFanSpeed(node){const d=rec(node);let s=Number(d.speed??d.fan_speed??d.fan?.speed??channelRecord(node,5).speed??0);return Number.isFinite(s)?Math.max(0,Math.min(4,s)):0}
function reconcileOptimistic(){
  Object.keys(optimisticState).forEach(node=>{const o=optimisticState[node];for(let i=1;i<=4;i++){if(o.relays[i]!==undefined){const updated=Number(channelRecord(node,i).state_updated_at||0)*1000;if(updated&&updated>=Number(o.relayAt[i]||0)){delete o.relays[i];delete o.relayAt[i]}}}if(o.speed!==null){const updated=Number(channelRecord(node,5).state_updated_at||0)*1000;if(updated&&updated>=Number(o.speedAt||0)){o.speed=null;o.speedAt=0}}})
}
...
async function command(channel,speed,status){
  ...
  const state=getOptimistic(node);const previous={relays:{...state.relays},relayAt:{...state.relayAt},speed:state.speed,speedAt:state.speedAt};const now=Date.now();
  if([1,2,3,4].includes(channel)){state.relays[channel]=status;state.relayAt[channel]=now}
  if(channel===5){state.speed=Number(speed);state.speedAt=now}
  if(channel===6)for(let i=1;i<=4;i++){state.relays[i]="ON";state.relayAt[i]=now}
  if(channel===7)for(let i=1;i<=4;i++){state.relays[i]="OFF";state.relayAt[i]=now}
  renderControl();
  try{
    const body={node_id:node,channel};
    if(channel===5){body.speed=Number(speed);body.status=Number(speed)===0?"OFF":"ON"}
    else if(speed!==null&&speed!==undefined)body.speed=speed;
    if(status)body.status=status;
    const r=await fetch("/api/device/control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),cache:"no-store"});const j=await r.json();
    if(!r.ok||j.status!=="success")throw new Error(j.message||"Command failed");
    setControlFeedback(channel===5?`${Number(speed)===0?"Fan OFF":"Fan S"+speed} sent`:`${status||"Command"} sent`,"success");
    setTimeout(()=>refresh(),350);
  }catch(e){optimisticState[node]=previous;renderControl();setControlFeedback(e.message||"Command failed","error")}
  finally{pendingCommands.delete(key);renderControl()}
}
function renderControl(){
  const node=$("controlNode").value||DATA.nodes[0]||DATA.all_nodes[0];if(!node)return;$("controlNode").value=node;
  const opt=getOptimistic(node);
  $("relayGrid").innerHTML=[1,2,3,4].map(i=>{const current=opt.relays[i]??actualRelay(node,i);const pending=pendingCommands.has(`${node}:${i}`);return `<div class="relay ${current==="ON"?"active":""}"><div><b>Relay ${i}</b><br><span class="muted">Channel ${i} · ${current}</span></div><button aria-label="Toggle Relay ${i}" class="switch ${current==="ON"?"on":""} ${pending?"pending":""}" ${pending?"disabled":""} onclick="toggleRelay(${i})"><span>${pending?"…":""}</span></button></div>`}).join("");
  const speed=opt.speed===null?actualFanSpeed(node):Number(opt.speed);const fanPending=pendingCommands.has(`${node}:5`);
  $("speedRow").innerHTML=[0,1,2,3,4].map(s=>`<button type="button" class="${speed===s?"primary":"secondary"} ${fanPending?"loading":""}" ${fanPending?"disabled":""} onclick="command(5,${s},null)">${s===0?"OFF":"S"+s}</button>`).join("");
  $("fanState").textContent=speed===0?"OFF":"SPEED "+speed;$("fanOrb").classList.toggle("spinning",speed>0);
}
async function toggleRelay(i){const node=$("controlNode").value,opt=getOptimistic(node),current=opt.relays[i]??actualRelay(node,i);await command(i,null,current==="ON"?"OFF":"ON")}
$("controlNode")?.addEventListener("change",()=>{const n=$("controlNode").value;getOptimistic(n);renderControl();setControlFeedback(`${deviceName(n)} selected`)});
...
function usbLog(msg){const el=$("usbStatus");if(el)el.textContent=msg}
...
function renderDiagnostics(){let n=$("diagNode").value||DATA.nodes[0]||DATA.all_nodes[0];if(!n){$("diagnosticPanel").innerHTML='<p class="muted">No device data.</p>';return}let d=rec(n),h=health(n);$("diagnosticPanel").innerHTML=`<div class="stats"><div class="stat"><span>Health</span><b>${h}%</b></div><div class="stat"><span>Status</span><b>${isOnline(n)?"ONLINE":"OFFLINE"}</b></div><div class="stat"><span>RSSI</span><b>${d.rssi??"—"}</b></div><div class="stat"><span>Firmware</span><b>${esc(d.fw_version||d.version||"Unknown")}</b></div></div><div class="alert">${isOnline(n)?"Network status is live from MQTT.":"No recent telemetry; device is considered offline."}</div><p>Boots: ${d.boot_count??0} · Crashes: ${d.crash_count??0} · Uptime: ${formatUptime(d.uptime)} · IP: ${esc(d.local_ip||"—")}</p>`}
